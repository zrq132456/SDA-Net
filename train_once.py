...
def train_once(args) -> Dict[str, Any]:
    ...
    if hasattr(args, "train_ids") and hasattr(args, "val_ids"):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            if args.use_dirichlet:
                # 1) Dirichlet belief supervision
                L_dir = -torch.log(P[y] + 1e-9)

                # 2) Evidence margin loss
                # correct-class evidence should be larger than the strongest wrong-class evidence
                e_hat = out["e_hat"].clamp(min=1e-8)

                wrong_mask = torch.ones(K, dtype=torch.bool, device=device)
                wrong_mask[y] = False

                pos_evi = e_hat[y]
                neg_evi = e_hat[wrong_mask].max()

                margin = 1.0
                L_margin = F.relu(margin - (pos_evi - neg_evi))

                # 3) Mild uncertainty regularization
                # avoid making this large, otherwise evidence growth will be suppressed
                uncertainty = K / alpha0
                L_unc = 0.0005 * uncertainty

                # final EDL loss
                L_cls = L_dir + 0.2 * L_margin + L_unc
            ...
            # T regularization: prevent excessive suppression
            L_conf = torch.mean(torch.relu(0.2 - gamma))

            # R regularization: prevent extreme risk scaling
            L_risk = torch.mean((tau - 1.0) ** 2)
